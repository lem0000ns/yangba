...
class MySQL_Writer:
    def __init__(self, table, season):
        ...
        if self.table == "version2":
            self.columns = ["name", "playerID", "gameID", "season", "points", "min", "fgm", "fga", "ftm", "fta", "3pm", "3pa", "reb", "ast", "steals", "blocks", "turnovers", "OPI"]
    ...
    def transferData(self):
        ...
        for game in data:
            values = [game.get(col, None) for col in self.olumns]
    ...
    def normalizeOPI(self):
        ...
        if self.table == "version2":
            ...
            cursor.execute(f"UPDATE version2 SET OPI = ROUND((OPI - {min_OPI}) / ({max_OPI} - {min_OPI}), 3) WHERE season={self.season}")
            self.connection.commit()
...
class DataDumper:
    ...
    def dumpData(self):
        if self.type == "names":
            ...
            threads = []
            for i in range(2):
                thread = threading.Thread(target=self.getAllPlayers, args=(self.season, allNames, 1 + 15 * i, True))
            ...
            self.dumpS3(allGames, f"gameStages/games{self.season}.json")
                ...
                for player in data:
                    for currGame in range(len(data[player])):
                        ...
                        if valid:
                            ...
                            self.calculateOPI(playerStats, gameID)
            ...
            self.dumpS3(allStats, f"version2/playerStats{self.season}.json")
